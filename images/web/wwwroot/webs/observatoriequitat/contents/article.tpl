...
{if (isset($vars.total) && $vars.total > 0) || sizeof($docs) > 0}
    ...
    {if $item.path != ''}
            <a href="{$item.url}" target="_blank">
                <img src="{$web_urlMedia}/docsArticle/{$content.id}/{$item.path}" class="responsive-img"
                    alt="{$item.descripcio}" style="min-width: 100%;" />
            </a>
        {else}
        ...
        <div class="container-detall">
            ...
            <div class="collection detall-item">
                {foreach name=documentsArticle item=item from=$docs.documents}
                    <a href="{$web_urlMedia}/docsArticle/{$content.id}/{$item.path}"
                        class="collection-item grey-text text-darken-3 " target="_blank">
